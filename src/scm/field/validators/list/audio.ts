/**
 * scm/validators/list/audio.ts
 * [Readme] --------------------------------------------------------------------------------------
 *
 * [audio] 타입에 대한 데이터 유효성 검사 클래스가 저장되어 있습니다.
...
/**
 * @description * audio 필드에 대한 데이터 유효성 검사 및 검증 클래스.
 */
@ValidatorDecorator("audio")
export class AudioValidator extends FileValidator {
    async validateFile(data: any, restrictions: Record<string, any>): Promise<boolean> {
        ...
        // 기초 파일 형태 검사.
        if (!this.fileMetadata.mimetype.startsWith("audio/")) {
            return false;
        }
