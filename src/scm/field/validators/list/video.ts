/**
 * scm/validators/list/video.ts
 * [Readme] --------------------------------------------------------------------------------------
 *
 * [video] 타입에 대한 데이터 유효성 검사 클래스가 저장되어 있습니다.
...
/**
 * @description * video 필드에 대한 데이터 유효성 검사 및 검증 클래스.
 */
@ValidatorDecorator("video")
export class VideoValidator extends FileValidator {
    async validateFile(data: any, restrictions: Record<string, any>): Promise<boolean> {
        ...
        // 기초 파일 형태 검사.
        if (!this.fileMetadata.mimetype.startsWith("video/")) {
            return false;
        }
